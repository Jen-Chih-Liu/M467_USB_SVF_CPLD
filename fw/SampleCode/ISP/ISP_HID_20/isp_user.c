...
#include <string.h>
#include "isp_user.h"
#define CMD_READ_FLASH              0x000000AA
...
int ParseCmd(uint8_t *pu8Buffer, uint8_t u8len)
{
    ...
    uint8_t *pu8Response;
    uint16_t u16Lcksum;
    uint32_t u32Lcmd, u32srclen, u32i, u32regcnf0, u32security, temp_cnt;
    uint8_t *pu8Src;
...
#if 0
    //new command for read flash
    else if (u32Lcmd == CMD_READ_FLASH)
    {
        read_Address = inpw((uint32_t)pu8Src);
        ReadData(read_Address, read_Address + 32, (uint32_t *)read_buffer); //read data from start address
        u16Lcksum = Checksum(read_buffer, 32);                 //get checksum from read buffer
        outpw(pu8Response + 12, u16Lcksum);                                             //write read data checksum
        outpw(pu8Response + 8, read_Address);                                 //write read_address to respond buffer

        for (temp_cnt = 0; temp_cnt < 32; temp_cnt++)                     //write read_buffer to respond buffer
        {
            pu8Response[16 + temp_cnt] = read_buffer[temp_cnt];
        }
