...
/**
 * @brief  Soft (bit-bang) I2C multi-byte read from one register, on PB4/PB5.
 * @details Mirrors the semantics of the BSP hardware I2C_ReadMultiBytesOneReg,
 *          but uses the GPIO bit-bang engine to avoid the hardware I2C
 *          Repeated START issue. Sequence:
 *            START -> SLA+W -> RegAddr -> Repeated START -> SLA+R
 *            -> read u32Len bytes (ACK each, NACK the last) -> STOP
 * @param  i2c_addr 7-bit slave address.
 * @param  reg      Register address to read from.
 * @param  data     Output buffer.
 * @param  u32Len   Number of bytes to read.
 * @return Number of bytes actually read (0 on address/register NACK).
 */
uint32_t SoftI2C_ReadMultiBytesOneReg(uint8_t i2c_addr, uint8_t reg, uint8_t *data, uint32_t u32Len)
{
    int ack;
    uint32_t rxLen = 0;
    ...
    /* Phase 2: Repeated START + SLA+R + read u32Len bytes */
    soft_i2c_start();   /* Repeated START */
    ack = soft_i2c_write_byte((uint8_t)((i2c_addr << 1u) | 0x01u));
    if (ack != 0) { soft_i2c_stop(); goto exit; }
    ...
    while (rxLen < u32Len)
    {
        /* NACK on the last byte, ACK on the others */
        int send_nack = (rxLen == (u32Len - 1u)) ? 1 : 0;
        data[rxLen++] = soft_i2c_read_byte(send_nack);
    }
...
/* --------------------------------------------------------------------------
 * Soft (bit-bang) I2C for I2C2 using PA10=SDA, PA11=SCL (open-drain style)
 * -------------------------------------------------------------------------- */

/* Release SDA = switch to input (external pull-up pulls high) */
static void soft_sda2_high(void)
{
    GPIO_SetMode(PA, BIT10, GPIO_MODE_INPUT);
}

/* Drive SDA low */
static void soft_sda2_low(void)
{
    PA10 = 0;
...
/* Release SCL = switch to input, then wait for clock stretching */
static void soft_scl2_high(void)
{
    GPIO_SetMode(PA, BIT11, GPIO_MODE_INPUT);
    /* Clock stretching: wait until SCL is actually high */
    uint32_t timeout = 10000u;
    while (PA11 == 0 && --timeout);
}

/* Drive SCL low */
static void soft_scl2_low(void)
{
    PA11 = 0;
    GPIO_SetMode(PA, BIT11, GPIO_MODE_OUTPUT);
}

static void soft_i2c2_start(void)
{
    soft_sda2_high();
    soft_scl2_high();
    soft_i2c_delay();
    soft_sda2_low();     /* SDA falls while SCL high = START */
    soft_i2c_delay();
    soft_scl2_low();
    soft_i2c_delay();
}

static void soft_i2c2_stop(void)
{
    soft_sda2_low();
    soft_i2c_delay();
    soft_scl2_high();
    soft_i2c_delay();
    soft_sda2_high();    /* SDA rises while SCL high = STOP */
    soft_i2c_delay();
}

/* Returns 0=ACK, 1=NACK */
static int soft_i2c2_write_byte(uint8_t byte)
{
    int i, ack;
...
/* send_nack=1 sends NACK after byte (use for last byte read) */
static uint8_t soft_i2c2_read_byte(int send_nack)
{
    uint8_t byte = 0;
    int i;

    soft_sda2_high();    /* Release SDA for slave to drive */
    ...
    soft_scl2_low();
    soft_i2c_delay();
    soft_sda2_high();    /* Release SDA */
    return byte;
}

/**
 * @brief Switch PA10/PA11 from hardware I2C2 to GPIO for bit-bang.
 */
static void soft_i2c2_enter(void)
{
    SYS_UnlockReg();
    /* Clear MFP to GPIO (MFP value 0x0 = GPIO on M467) */
    SYS->GPA_MFP2 = (SYS->GPA_MFP2
                     & ~(SYS_GPA_MFP2_PA10MFP_Msk | SYS_GPA_MFP2_PA11MFP_Msk));
    SYS_LockReg();
    /* Initial state: both lines high (idle) */
    soft_sda2_high();
    soft_scl2_high();
...
/**
 * @brief Restore PA10/PA11 to hardware I2C2.
 */
static void soft_i2c2_exit(void)
{
    SYS_UnlockReg();
    SET_I2C2_SDA_PA10();
    SET_I2C2_SCL_PA11();
...
/**
 * @brief  Soft (bit-bang) I2C multi-byte read from one register, on PA10/PA11 (I2C2).
 * @details Mirrors SoftI2C_ReadMultiBytesOneReg but drives the I2C2 pins.
 *          Sequence:
 *            START -> SLA+W -> RegAddr -> Repeated START -> SLA+R
 *            -> read u32Len bytes (ACK each, NACK the last) -> STOP
 * @param  i2c_addr 7-bit slave address.
 ...
 * @return Number of bytes actually read (0 on address/register NACK).
 */
uint32_t SoftI2C2_ReadMultiBytesOneReg(uint8_t i2c_addr, uint8_t reg, uint8_t *data, uint32_t u32Len)
{
    int ack;
    uint32_t rxLen = 0;
    ...
    /* Phase 1: START + SLA+W + register address */
    soft_i2c2_start();
    ack = soft_i2c2_write_byte((uint8_t)((i2c_addr << 1u) | 0x00u));
    if (ack != 0) { soft_i2c2_stop(); goto exit; }

    ack = soft_i2c2_write_byte(reg);
    if (ack != 0) { soft_i2c2_stop(); goto exit; }

    /* Phase 2: Repeated START + SLA+R + read u32Len bytes */
    soft_i2c2_start();   /* Repeated START */
    ack = soft_i2c2_write_byte((uint8_t)((i2c_addr << 1u) | 0x01u));
    if (ack != 0) { soft_i2c2_stop(); goto exit; }
    ...
    while (rxLen < u32Len)
    {
        /* NACK on the last byte, ACK on the others */
        int send_nack = (rxLen == (u32Len - 1u)) ? 1 : 0;
        data[rxLen++] = soft_i2c2_read_byte(send_nack);
    }
